...
- **规范**：
  - 使用 MyBatis Plus 注解
  - 字段与数据库表字段对应
  - 常用的连表字段可以在entity写属性接收，用@TableField(exist = false)注解标识，这样以便类型判断。不常用的可以用BaseEntity里的param属性接收
...
项目使用自动化资源扫描机制，通过 `@PreAuthorize` 注解自动生成资源权限记录。
...
**注解说明：**
- `@Tag(name = "用户管理")` - 定义所属模块，扫描器会提取此值作为资源的模块名称
- `@Operation(summary = "创建用户", description = "创建新用户")` - 定义资源名称和描述
- `@PreAuthorize("hasAuthority('user:create')")` - 定义权限码，扫描器会自动提取

**支持的权限表达式格式：**
```java
// 1. 使用 hasAuthority（推荐）
@PreAuthorize("hasAuthority('user:create')")

// 2. 直接写权限码
@PreAuthorize("'user:create'")
```

**注意：**
- 只支持以上两种简单格式
- 不支持 `hasAnyAuthority`、`hasRole`、`and`、`or` 等复杂表达式
- 如需复杂权限控制，请在业务代码中实现

**自动化资源扫描：**
1. 项目启动时，`ResourceScanner` 自动扫描所有带 `@PreAuthorize` 注解的接口
2. 从 `@Tag` 提取模块名称
3. 从 `@Operation` 提取资源名称和描述
4. 从 `@PreAuthorize` 提取权限码
5. 自动提取 URL 和 HTTP 方法
6. 自动新增或更新资源记录到数据库
7. 自动删除孤儿资源（数据库中存在但代码中已删除的接口）

**资源同步策略：**
- **新增资源**：扫描到新的权限码时，自动创建资源记录
- **更新资源**：扫描到已存在的权限码时，自动更新模块、URL、HTTP方法（保持用户自定义的名称和描述）
- **删除资源**：数据库中的资源如果未被扫描到，说明对应接口已删除，自动删除该资源记录
...
charter:
  security:
    resource:
      scan-enabled: true  # 是否启用资源扫描，默认true
```

**注意事项：**
- 资源由系统自动管理，不支持手动新增和删除
- 只能在资源管理页面编辑资源名称和描述
- 权限码必须唯一，建议使用统一命名规范
- 删除接口后，对应资源会在下次启动时自动清理
- 更新资源时会保留用户自定义的名称和描述，只更新系统字段（模块、URL、HTTP方法）

...
import ink.charter.website.common.file.utils.CharterFileUtils;
...
public class FileServiceExample {
    ...
    public void uploadFileExample(MultipartFile file) {
        try {
            // 方式1：使用工具类上传到指定路径
            SysFilesEntity fileEntity = CharterFileUtils.upload(file, "uploads/images");
            log.info("文件上传成功，文件ID: {}, 文件名: {}", fileEntity.getId(), fileEntity.getFileName());
            
            // 方式2：使用默认路径
            SysFilesEntity fileEntity2 = CharterFileUtils.upload(file);
    ...
    public void checkFileExistsExample(MultipartFile file) {
        try {
            // 计算文件MD5
            String fileMd5 = CharterFileUtils.calculateMd5(file);
            
            // 检查文件是否已存在
            if (CharterFileUtils.exists(fileMd5)) {
                log.info("文件已存在，可以实现秒传功能");
                SysFilesEntity existingFile = CharterFileUtils.getByMd5(fileMd5);
    ...
    public void getFileInfoExample(Long fileId) {
        try {
            // 根据ID获取文件信息
            SysFilesEntity fileEntity = CharterFileUtils.getById(fileId);
            if (fileEntity != null) {
                log.info("文件名: {}", fileEntity.getFileName());
                log.info("文件大小: {}", CharterFileUtils.formatFileSize(fileEntity.getFileSize()));
                log.info("文件类型: {}", fileEntity.getFileType());
                log.info("上传时间: {}", fileEntity.getUploadTime());
                
                // 判断文件类型
                if (CharterFileUtils.isImageType(fileEntity.getFileType())) {
                    log.info("这是一个图片文件");
                } else if (CharterFileUtils.isVideoType(fileEntity.getFileType())) {
                    log.info("这是一个视频文件");
                } else if (CharterFileUtils.isDocumentType(fileEntity.getFileType())) {
                    log.info("这是一个文档文件");
                }
            }
    ...
    public void getCurrentUserFilesExample() {
        try {
            List<SysFilesEntity> userFiles = CharterFileUtils.getCurrentUserFiles();
            log.info("当前用户共有 {} 个文件", userFiles.size());
            
            for (SysFilesEntity file : userFiles) {
                log.info("文件: {} ({})", file.getFileName(), 
                        CharterFileUtils.formatFileSize(file.getFileSize()));
            }
    ...
    public void getFilesByTypeExample(String fileType) {
        try {
            List<SysFilesEntity> files = CharterFileUtils.getByType(fileType);
    ...
    public void batchDeleteFilesExample(List<Long> fileIds) {
        try {
            Integer deletedCount = CharterFileUtils.deleteFiles(fileIds);
    ...
    public void validateFileNameExample(String fileName) {
        if (CharterFileUtils.isValidFileName(fileName)) {
    ...
    public void controllerUsageExample() {
        ...
        public class FileController {
            ...
            public Result<FileVO> uploadFile(@RequestParam("file") MultipartFile file,
                ...
                // 上传文件
                SysFilesEntity fileEntity = CharterFileUtils.upload(file, path, currentUserId);
                
                // 转换为VO返回
                FileVO fileVO = new FileVO();
                fileVO.setId(fileEntity.getId());
                fileVO.setFileName(fileEntity.getFileName());
                fileVO.setFileSize(CharterFileUtils.formatFileSize(fileEntity.getFileSize()));
                fileVO.setFileType(fileEntity.getFileType());
                fileVO.setDownloadUrl(CharterFileUtils.getDownloadUrl(fileEntity.getId()));
